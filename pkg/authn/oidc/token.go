...
func NewAccessTokenClaims(issuer, subject string, audience []string, expiration time.Time, jwtid, clientID string) *AccessTokenClaims {
	now := time.Now().UTC()
	if len(audience) == 0 {
...
func NewIDTokenClaims(
	sessionID, issuer, subject string,
	audience []string,
	expiration, authTime time.Time,
	clientID string,
) *IDTokenClaims {
