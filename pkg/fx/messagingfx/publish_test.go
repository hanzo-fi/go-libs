...
func TestModule(t *testing.T) {
	...
	testCases := []moduleTestCase{
		{
			name: "go-channels",
			setup: func(t *testing.T) fx.Option {
				return messagingfx.GoChannelModule()
			},
			topic: "topic",
		},
		{
			...
			setup: func(t *testing.T) fx.Option {
				...
				return fx.Options(
					messagingfx.NatsModule("nats://127.0.0.1:4322", "testing", true, nats.Name("example")),
				)
			},
			topicMapping: map[string]string{},
			topic:        "topic",
	...
	for _, tc := range testCases {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			options := []fx.Option{
				messagingfx.Module(tc.topicMapping),
				tc.setup(t),
